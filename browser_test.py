...
def wait_ready(driver):
    ...
    WebDriverWait(driver, 10).until(lambda d: js(d, "return typeof startMock==='function' && typeof mock!=='undefined'"))


def data_contract_test(driver):
    data = js(driver, "return mock.map(q=>({unit:q.unit,type:q.type||'choice',answer:q.answer,options:q.options||[],diagram:q.diagram||'',rubric:q.rubric||'',steps:q.steps||[],trap:q.trap||'',retry:q.retry||''}))")
    ...
    check(all(len(q["options"]) == 5 and len(set(q["options"])) == 5 for q in data[:20]), "객관식 보기 5개 및 중복 없음")
    check(all(len(q["steps"]) >= 2 and q["trap"] and q["retry"] for q in data), "전 문항 해설·오답·재시험 기준")
    ...
    for n, q in enumerate(data, 1):
        ...
        check(all(s.get("role") == "img" and s.get("aria-label", "").strip() for s in svgs), f"{n}번 SVG role/한국어 aria-label")
        check(all(any("가" <= c <= "힣" for c in s.get("aria-label", "")) for s in svgs), f"{n}번 SVG 한국어 설명")


def interaction_test(driver, base):
    driver.get(base)
    wait_ready(driver)
    js(driver, "localStorage.clear(); mockAnswers=Array(25).fill(null); mockFlags=Array(25).fill(false); mockIndex=0; mockLeft=7200; mockSubmitted=false; startMock(); clearInterval(mockInterval)")
    check(js(driver, "return mockIndex") == 0, "실전 1번 시작")
    js(driver, "showSolutions()")
    check("실전 제출 후 공개" in driver.find_element(By.ID, "solutionList").text, "실전 제출 전 정오·해설 차단")
    js(driver, "showView('mock'); renderMock()")
    click(driver, "mockNext", By.ID)
    check(js(driver, "return mockIndex") == 1 and js(driver, "return mockAnswers[0]") is None, "무응답 다음 이동")
    ...
    check(js(driver, "return mockFlags[9]") is True and "검토" in driver.find_element(By.ID, "flagBtn").text, "검토 표시/해제 상태")
    ...
    # 정답 전체 입력 및 100점 채점
    js(driver, "mockAnswers=mock.map(q=>q.type==='choice'?q.answer:q.accept[0]); mockFlags=Array(25).fill(false); saveState(); openSubmitReview()")
    ...
    check(js(driver, "return warmIndex") == 1 and js(driver, "return warmAnswers[0]") is None, "학습 무응답 다음 이동")
...
def persistence_test(driver, base):
    driver.get(base)
    wait_ready(driver)
    js(driver, "localStorage.clear(); startMock(); clearInterval(mockInterval); answerMock(1); jumpMock(4); toggleFlag(); mockLeft=7011; saveState()")
    driver.refresh()
    wait_ready(driver)
...
def keyboard_mobile_print_test(driver, base):
    ...
    check(js(driver, "return document.documentElement.scrollWidth <= document.documentElement.clientWidth"), "모바일 가로 스크롤 없음")
    click(driver, "mobileMapToggle", By.ID)
    ...
    wait_ready(driver)
    check(js(driver, "return document.querySelectorAll('#printRoot .print-question').length") == 25, "인쇄 문제지 25문항")
    check("객관식 1~20" in driver.find_element(By.ID, "printRoot").text and "과정형 24~25" in driver.find_element(By.ID, "printRoot").text, "인쇄 응답 유형 구분")
    check(js(driver, "return document.querySelectorAll('#printRoot .process-space .write-line').length") >= 10, "과정형 문항당 풀이 공간 5줄 이상")
    driver.execute_cdp_cmd("Emulation.setEmulatedMedia", {"media": "print"})
    driver.save_screenshot(str(ARTIFACTS / "print-exam.png"))

    driver.get(base + "?print=solutions&test=1")
    wait_ready(driver)
    check(js(driver, "return document.querySelectorAll('#printRoot .print-sol').length") == 25, "인쇄 해설 25문항")
    check(js(driver, "return document.querySelectorAll('#printRoot .print-sol svg').length") >= 18, "인쇄 해설 시각자료 포함")
...
def static_accessibility_test():
    ...
    check("window.alert" not in HTML.read_text(encoding="utf-8") and "confirm(" not in HTML.read_text(encoding="utf-8"), "native alert/confirm 미사용")
    check(bool(soup.select_one("@media print")) if False else "@media print" in HTML.read_text(encoding="utf-8"), "인쇄 CSS 존재")
    check("filter:grayscale(1)" in HTML.read_text(encoding="utf-8") and "stroke-dasharray" in HTML.read_text(encoding="utf-8"), "흑백 인쇄 및 선종류 구분")
    for name in ("middle-math-70-exam.pdf", "middle-math-70-solutions.pdf"):
        pdf = ROOT / name
        check(pdf.exists() and pdf.stat().st_size > 100_000 and pdf.read_bytes()[:5] == b"%PDF-", f"기존 PDF 파일명 계약 및 유효 PDF: {name}")
